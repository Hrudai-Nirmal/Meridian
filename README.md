...
Control Room also includes Interactive Tutorial v1 for the first workflow path. It auto-starts once per browser for projects with no runs or metric samples, uses a click-through guide so users can operate the app while following the tutorial, guides users across Automation Map, Integrations, telemetry testing, Runs, and Reports, and can be restarted from Control Room. The tutorial shows an evidence-backed progress bar for node setup, first run/sample, and report proof; `Check progress` refreshes project evidence. Tutorial completion is stored locally as `meridian-tutorial:first-workflow:v1` with `completed` or `skipped`; no database migration is required.
