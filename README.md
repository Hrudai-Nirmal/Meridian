...
Control Room also includes Interactive Tutorial v1 for the first workflow path. It auto-starts once per browser for projects with no runs or metric samples, blocks background app clicks while the tutorial is active, guides users across Automation Map, Integrations, telemetry testing, Runs, and Reports, and can be restarted from Control Room. The tutorial shows an evidence-backed progress bar for node setup, first run/sample, and report proof; `Check progress` refreshes project evidence. Tutorial completion is stored locally as `meridian-tutorial:first-workflow:v1` with `completed` or `skipped`; no database migration is required.
...
- Interactive Tutorial v1 auto-starts for no-telemetry projects, blocks background app clicks, can be skipped/finished, can be restarted from Control Room, shows evidence progress, and navigates through Map, Integrations, Runs, and Reports without exposing secrets.
