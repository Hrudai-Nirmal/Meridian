...
- Added First Workflow Launchpad in Control Room: an evidence-backed four-step checklist for creating/selecting a node, connecting telemetry or metric polling, verifying ops evidence, and sharing client proof. Step state is derived from safe project counts only.
...
- Browser-test First Workflow Launchpad states: blank projects should start at create/select node, node-only projects should point to Integrations, telemetry-ready projects should point to Reports, and active report links should mark the checklist complete.
