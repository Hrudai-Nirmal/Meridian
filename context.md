...
- Added Python and JavaScript SDK previews in `sdk/python` and `sdk/js`, plus `docs/sdk.md`, using the existing `/api/ingest/runs` telemetry contract. SDK onboarding now includes local validation, configurable delivery timeouts, Python `on_error`, JavaScript `onError`, and JavaScript `flush()` for short-lived scripts/serverless handlers.
