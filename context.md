...
- SSE live updates are implemented as a lightweight signal channel; anomaly alerting, generic outbound alert webhooks, native Slack incoming-webhook notifications, durable notification jobs, and audit-backed operational logs are implemented for metric incidents and operator actions; public SDK publishing remains a next-stage priority after package/account ownership is intentionally set up.
...
- Test SDK preview snippets and package READMEs against a disposable ingestion token and confirm runs appear in the selected node.
