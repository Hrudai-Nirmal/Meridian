...
- Replaced the First Workflow Launchpad with Interactive Tutorial v1: a browser-local guided overlay for the first workflow path across Automation Map, Integrations, telemetry test, Runs, and Reports. It auto-starts once for no-telemetry projects, can be restarted from Control Room, and stores `completed`/`skipped` in localStorage.
...
- SSE live updates are implemented as a lightweight signal channel; anomaly alerting, generic outbound alert webhooks, native Slack incoming-webhook notifications, durable notification jobs, and audit-backed operational logs are implemented for metric incidents and operator actions; Python SDK publishing remains a next-stage priority after PyPI project/account ownership is intentionally set up.
...
- Browser-test Interactive Tutorial v1: clear localStorage and confirm no-telemetry projects auto-start; verify Skip/Finish suppress future auto-starts; restart from Control Room; confirm steps navigate Map, Integrations, Runs, and Reports; confirm missing targets fall back cleanly and no secrets appear.
